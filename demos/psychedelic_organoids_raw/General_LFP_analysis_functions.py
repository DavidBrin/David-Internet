...
def fooof_all_pspectra(ds_wells_data, fs_ds = 100, fmode = "knee",n_rows = 6, n_cols = 8 ):
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            ...
            fm = FOOOF(min_peak_height=0.6, peak_width_limits= (4, 15),  aperiodic_mode = fmode, peak_threshold= 0.6)
            freq_range = [1, 50]
...
def set_fm_array(ds_wells_data, fs_ds = 100, fmode = "knee", n_rows = 6, n_cols = 8):
    ...
    for i in range(n_rows):
        for j in range(n_cols):
            ...
            fm = FOOOF(min_peak_height=0.6, peak_width_limits= (4, 15),  aperiodic_mode = fmode, peak_threshold= 0.6, verbose= False)
            fm_array[i, j] = fm
...
def isi_array(spike_times_array):
    """
    Computes inter-spike intervals (ISI) for each electrode in a 6x8x4x4 electrode grid.

    For each electrode that contains valid spike times, the function computes the differences 
    between consecutive spike times (i.e., ISIs), and stores them in an array of the same 
    shape (6x8x4x4), where each element contains a 1D NumPy array of ISIs. Electrodes with 
    no valid spikes will contain None.

    Parameters:
    - spike_times_array: ndarray
        A 6x8x4x4x1xN NumPy array representing the spike times recorded from a multi-well 
        electrode array. The spike times for each electrode are stored at [row, col, i, j, 0].

    Returns:
    - isi_arr: ndarray
        A 6x8x4x4 array where each element is:
        - A NumPy array of inter-spike intervals for that electrode, or
        - None if no valid spikes were recorded.
    """
    ...
    for row in range(6):
        for col in range(8):
            for i in range(4):
                for j in range(4):
                    if (
                        spike_times_array[row, col, i, j] is not None
                        and spike_times_array[row, col, i, j].size > 0
                    ):
                        spikes = spike_times_array[row, col, i, j][0]
                        isi = np.diff(spikes)
                        isi_arr[row, col, i, j] = isi
                    else:
...
def burst_rate(isi_arr, isi_thresh=1.0, min_spikes=3):
    """
    Calculates the burst rate for each electrode based on inter-spike intervals.

    A burst is defined as a sequence of at least `min_spikes` spikes (i.e., at least 
    `min_spikes - 1` consecutive ISIs) where each ISI is below the specified `isi_thresh`.
    The function iterates through all ISIs in the array, counts how many such bursts occur 
    in each electrode, and stores the count in a corresponding array.

    Parameters:
    - isi_arr: ndarray
        A 6x8x4x4 array where each element is a NumPy array of inter-spike intervals 
        for a given electrode, as returned by `isi_array`.
    - isi_thresh: float (default=1)
        The maximum ISI (in seconds) that qualifies as part of a burst.
    - min_spikes: int (default=3)
        The minimum number of spikes needed to define a burst. Corresponds to at least 
        `min_spikes - 1` ISIs below the threshold.

    Returns:
    - burst_array: ndarray
        A 6x8x4x4 array where each value represents the number of bursts detected 
        for that electrode.
    
    Side Effects:
    - Displays a heatmap showing burst counts across the entire electrode grid.
    """
    ...
    burst_heatmap = np.sum(burst_counts, axis=(2, 3))  # Summarize by well
    plt.figure(figsize=(12, 6))
    sns.heatmap(burst_heatmap, cmap='viridis', annot=True, fmt='d')
    plt.title("Burst Count per Well")
    plt.xlabel("Column")
    plt.ylabel("Row")
    plt.show()
...
def network_events(spike_times_array, isi_thresh=1, min_spikes=3):
    """
    Detects network events in a 6x8 array of wells based on synchronized spiking activity.
    
    A network event is defined as at least `min_spikes` spikes from different electrodes 
    in the same well that occur within `isi_thresh` seconds of each other.

    Parameters:
    - spike_times_array: 6x8x4x4x1xN ndarray containing spike times.
    - isi_thresh: float, time window (in seconds) to consider spikes as part of the same event.
    - min_spikes: int, minimum number of spikes from unique electrodes to count as a network event.

    Returns:
    - network_event_array: 6x8 array with event counts per well.
    """
    network_event_array = np.zeros((6, 8), dtype=int)

    for row in range(6):
        for col in range(8):
            # Step 1: Gather all spike events from this well (time, electrode)
            spike_events = []
            for i in range(4):
                for j in range(4):
                    if spike_times_array[row, col, i, j] is not None and spike_times_array[row, col, i, j].size > 0:
                        times = spike_times_array[row, col, i, j][0]
                        for t in times:
                            spike_events.append((t, (i, j)))

            # Step 2: Sort all events by time
            spike_events.sort()
            used_indices = set()  # track spikes already used in a network event
            event_count = 0
            i = 0

            while i < len(spike_events):
                if i in used_indices:
                    i += 1
                    continue

                window_start_time = spike_events[i][0]
                involved_electrodes = {spike_events[i][1]}
                involved_indices = {i}
                j = i + 1

                while j < len(spike_events) and spike_events[j][0] - window_start_time <= isi_thresh:
                    involved_electrodes.add(spike_events[j][1])
                    involved_indices.add(j)
                    j += 1

                if len(involved_electrodes) >= min_spikes:
                    event_count += 1
                    used_indices.update(involved_indices)  # mark all used spikes
                    i = max(involved_indices) + 1  # jump past the last used index
                else:
                    i += 1

            network_event_array[row, col] = event_count

    # Step 3: Plot heatmap of network events
    plt.figure(figsize=(8, 6))
    plt.imshow(network_event_array, cmap='magma', interpolation='nearest')
    plt.title("Network Events per Well")
