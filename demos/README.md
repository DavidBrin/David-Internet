...
| `dtu_databases_raw/` | SQL schema design and queries (`UniversityDB.sql`, `FamilyDB.sql`, `cinema.sql`, weekly answer sets) with hand-drawn ER diagrams (`Meeting room database design.png`, `News Items database design.png`, `Data Sheet.png`). | **Built into /demos/sql** (2026-09-01): six DBs live in sql.js, 44 fixture-tested presets, reconstructed bike-shop schema. Installers (100 MB of MariaDB/Workbench MSIs) and lecture PDFs excluded. |
...
| Folder | What it is | Why it's included |
|---|---|---|
| `cpp_2021_raw/` | 2021 C++ coursework: hw1–hw10 + final (`main.cpp` each), file-I/O + algorithm final over `numbers-*.txt` datasets, Doxygen configs. | **Built into /demos/earlycode** (2026-09-01): the final runs in a fake terminal (TS port, quirks preserved, fixture-tested). |
| `java_servers_raw/` | `wavelet_chat_server/ChatServer.java` (from-scratch socket HTTP chat server; note the README references `Server.java` which is missing from that copy), `doc_search_server/` (complete document-search HTTP server), `junit_lab/` (JUnit tests). | **Built into /demos/earlycode** (2026-09-01): handlers replayed in TS; wavelet credited as course-provided; synthetic corpus stands in; JUnit bug widget; cse12/ gathered here at build (scrubbed). |
| `misc_snippets_raw/` | `aho_corasick_string_matching.ipynb` (CSE 100, multi-pattern matching) and `CardClassifier.ipynb` (the one non-boilerplate PyTorch tutorial: a playing-card image classifier). | **Built into /demos/earlycode** (2026-09-01): from-scratch Aho-Corasick (the notebook's exact 106 nodes); CardClassifier referenced only (dataset not on disk). |
