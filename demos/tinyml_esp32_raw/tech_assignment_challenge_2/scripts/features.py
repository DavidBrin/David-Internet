...
def compute_spatial_features(raw_row):
    ...
    spatial_gradient = float((horizontal + vertical) / 2.0)
    ...
    q_tl = np.mean(grid[:4, :4])
    q_tr = np.mean(grid[:4, 4:])
    q_bl = np.mean(grid[4:, :4])
    q_br = np.mean(grid[4:, 4:])
    quadrant_var = float(np.var([q_tl, q_tr, q_bl, q_br]))
    ...
    center_mean = np.mean(grid[2:6, 2:6])
    center_sum = np.sum(grid[2:6, 2:6])
    outer_sum = np.sum(grid) - center_sum
    edge_mean = outer_sum / 48.0
    ...
    row_maxs = np.max(grid, axis=1)
    col_maxs = np.max(grid, axis=0)
    row_profile_std = float(np.std(row_maxs))
    col_profile_std = float(np.std(col_maxs))
    ...
    if hot_count > 0:
        ...
        hot_centroid_r = float(np.sqrt((centroid_r - 3.5) ** 2 + (centroid_c - 3.5) ** 2))
...
def engineer_features(df):
    ...
    count_above_3 = (pixels > (medians + 3.0)).sum(axis=1).reshape(-1, 1).astype(np.float32)
    count_above_5 = (pixels > (medians + 5.0)).sum(axis=1).reshape(-1, 1).astype(np.float32)
