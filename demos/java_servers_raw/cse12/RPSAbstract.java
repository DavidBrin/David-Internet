...
public abstract class RPSAbstract implements RPSInterface {
    ...
    public boolean isValidMove(String move) {
        ...
        for(int i = 0; i < possibleMoves.length; i ++){
                if(move.equals(possibleMoves[i])) return true;
        }
    ...
    public void playRPS(String playerMove, String cpuMove) {
        ...
        int win = determineWinner(playerMove, cpuMove);
        if(win == CPU_WIN_OUTCOME){ System.out.printf(CPU_MOVE+ CPU_WIN + "\n", cpuMove ); numCPUWins ++;}
        else if(win == PLAYER_WIN_OUTCOME){ System.out.printf(CPU_MOVE + PLAYER_WIN+ "\n", cpuMove); numPlayerWins ++;}
        else if(win == TIE_OUTCOME) {System.out.printf(CPU_MOVE + TIE + "\n", cpuMove); numTies++;}
        else System.out.printf(INVALID_INPUT);
